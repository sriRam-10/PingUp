...
const StoryViewer = ({viewStory,setViewStory}) => {
    ...
    progressInrerval =    setInterval(() => {
     ...
     return ()=>{
        clearTimeout(timer);
        clearInterval(progressInrerval);
     }
        ...
        switch (viewStory.media_type) {
            case 'image':
                 return( 
                  ...
                  case 'video':
                 return( 
                    <video onEnded={()=>setViewStory(null)} src={viewStory.media_url} className='max-w-full max-h-screen object-contain'
                      controls autoPlay/>
                 );
                  case 'text':
                 return( 
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-100 flex items-center justify-center'
      style={{backgroundColor: viewStory.media_type ==='text'? viewStory.background_color:'#000000'}}>
     {/* progress bar */}
     <div className='absolute top-0 left-0 w-full h-1 bg-gray-700'>
     <div className='h-full bg-white trsnsition-all duration-100 linear'
     ...
     <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4
                      sm:p-4 sm:px-8 backdrop-blur-2x1 rounded bg-black/50'>
