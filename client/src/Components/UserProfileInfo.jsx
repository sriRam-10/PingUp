...
const UserProfileInfo = ({user,posts,profileId,setShowEdit}) => {
  return (
    ...
    <div className='w-full pt-16 md:pt-0 md:pl-32'>
       <div className='flex flex-col md:flex-row items-start justify-between'>
         <div>
             <div className='flex items-center gap-6'>
               ...
               <p className='text-gray-600'>{user.username ? `@${user.username}`: 'Add a usernme'}</p>
         </div>
         {/* not adding edit button for the other people profiles */}
       {!profileId && 
           <button onClick={()=>setShowEdit(true)} className='flex items-center gap-2 border border-gray-300 hover:bg-gray-50 
                               px-4 py-3 rounded-lg font-medium transition-colors mt-4 md:mt-0 cursor-pointer'>
       ...
       <p className='text-gray-700 text-sm max-wid-md mt-4'>{user.bio}</p>
      ...
      <span className='flex items-center gap-1.5'>
       ...
       <div className='flex items-center mt-6 gap-6 border-t border-gray-200 pt-4'>
         <div>
