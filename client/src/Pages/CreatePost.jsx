...
{/* title */}
  <div className='mb-8'>
    <h1 className='text-3xl font-bold text-sate-900 mb-2'>Create post</h1>
 ...
 <div className='flex items-center gap-3'>
    ...
    <div>
      <h2 className='font-semi-bold'>{user.full_name}</h2>
   ...
   {/* text */}
    <textarea className='w-full resize-none max-h-20 mt-4 text-sm outline-none placeholder-gray-400'
               onChange={(e)=>setContent(e.target.value)} value={content} placeholder="What's Happening?"/>
    ...
    images.length > 0 && <div className='flex flex-wrap gap-2 mt-4'>
    {
        images.map((image,i)=>(
         <div key={i} className='relative group'>
          ...
          <div onClick={()=>setImages(images.filter((_,index)=>index !== i))} className='absolute hidden group-hover:flex justify-center items-center top-0 right-0 bottom-0
                           left-0 bg-black/40 rounded-mg cursor-pointer' >
 ...
 <Image className='size-6'/>
      ...
      <button   onClick={()=>toast.promise(
                  ...
                  {
                  loading : "Uploading...",
                  success : <p>Post Uplaoded Successfully</p>,
              ...
              className='text-sm bg-gradient-to-r from-indigo-500 to-purple-600
                          ...
                          text-white font-medium px-8 py-2 rounded-md cursor-pointer'>Publish post </button>
