  ...
  return (
    <div className='min-h-screen bg-slate-50' >
     <div className='max-w-6xl max-auto p-6'>
      {/* title */}
       <div className='mb-8'>
            <h1 className=' font-bold text-3xl text-slate-900 mb-2'>Connections</h1>
         ...
         {dataArray.map(( item,index)=>(
          ...
          <div className='inline-flex flex-wrap items-center border border-gray-200 
                          rounded-mg bg-white p-1 shadow-sm'>
           {dataArray.map((tab)=>(
          ...
          <div className='flex flex-wrap gap-6 mt-6'>
             {dataArray.find((item) => item.label === currentTab).value.map((user)=>(
              <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-mg'>
                      ...
                      <div className='flex max-sm:flex-col gap-2 mt-4'>
                       {
                         <button onClick={()=>navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-gradient-to-r
                                           ...
                                           hover:to-purple-700 scale-95 transition text-white cursor-pointer'>
                           View Profile
                        </button>
                       }
