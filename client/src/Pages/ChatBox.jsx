...
const ChatBox = () => {
  ...
  return user && (
    <div className='flex flex-col h-screen'>
       <div className='flex items-center-gap2 p-2 md:px-10 xl:pl-42
                        bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-gray-300'>
             <img src={user.profile_picture} alt="" className='size-8 rounded-full' />
             <div>
              <p className='font-medium'>{user.full_name}</p>
              <p className='text-smtext-gay-500 -mt-1.5'>@{user.username}</p>
             </div>
       </div>
       <div className='p-5 md:px-10 h-full overflow-y-scroll'>

<div className='space-y-4 mxw-4xl mx-auto'>
{
...
<div ref={messagesEndRef}/>
       ...
       <div className='px-4'>
        <div className='flex items-center gap-3 pl-1.5 p-1.5 bg-white w-full max-w-xl mx-auto border bodrer-gray-200
                         rounded-full shadow mb-5 '>
        <input type="text" 
               className='flex-1 outline-none text-slate-700 '
                ...
                <label htmlFor="image">
                  ...
                  {image ? <img src={URL.createObjectURL(image)} alt="" className='rounded h-8' /> : 
